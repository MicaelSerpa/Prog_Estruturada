...
void askStrUser(char dispositivos[5][100], int num_Strings) {
    ...
    if (aux < num_Strings) {
        ...
        system("cls");  // Limpa a tela (pode variar de acordo com o sistema operacional)
        aux++;
        askStrUser(dispositivos, num_Strings);
    } else {
        aux = 0; // Resetar o índice para uso futuro
    }
}
...
int i,j,k,y;
char dispositivos[5][100]={"1", "2", "3", "4", "5"};
float potencia[20];
float horas[20];
...
float soma_diario;
   ...
   scanf("%d", &num_Strings);
    ...
    initVetores(potencia, horas, dispositivos, num_Strings );
